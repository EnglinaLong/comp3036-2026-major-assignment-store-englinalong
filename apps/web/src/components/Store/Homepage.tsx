...
export async function StoreHomepage({ posts }: { posts: Post[] }) {
  ...
  return (
    <div className="space-y-14 pb-14">
      <section className="relative overflow-hidden rounded-[36px] bg-[linear-gradient(135deg,#fff7ed_0%,#ffffff_35%,#fee2e2_100%)] px-6 py-10 shadow-[0_25px_80px_rgba(127,29,29,0.12)] ring-1 ring-black/5 sm:px-8 lg:px-10 lg:py-12">
        ...
        <div className="relative grid gap-8 xl:grid-cols-[1.1fr_0.9fr] xl:items-center">
          <div className="space-y-6">
            <span className="inline-flex rounded-full border border-black/10 bg-white/80 px-4 py-2 text-xs font-semibold uppercase tracking-[0.28em] text-neutral-600">
              Full Stack Store
            </span>
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              ...
              <a
                ...
                className="inline-flex items-center justify-center rounded-full border border-neutral-300 bg-white/80 px-6 py-3 font-medium text-neutral-900 transition hover:-translate-y-0.5 hover:border-neutral-400 hover:bg-white"
              >
                Browse Categories
              </a>
            </div>
          ...
          <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-1">
            <div className="grid gap-4 sm:grid-cols-3 xl:grid-cols-3">
              <div className="rounded-[24px] bg-white/85 p-5 shadow-sm ring-1 ring-black/5">
                <p className="text-sm text-neutral-500">Available products</p>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-3 xl:grid-cols-1">
                {spotlightItems.map((item) => (
                  <Link
                    ...
                    className="rounded-[20px] border border-black/5 bg-white/90 p-4 transition hover:-translate-y-1 hover:shadow-md"
                  >
                    <p className="text-xs font-semibold uppercase tracking-[0.24em] text-neutral-500">
                      {item.category}
                    </p>
                    <p className="mt-2 line-clamp-2 text-base font-semibold text-neutral-950">
                      {item.title}
                    </p>
                  </Link>
                ))}
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {categoryItems.map((item) => (
            <Link
              ...
              className="hover:border-[color:var(--color-wsu)]/20 group rounded-[24px] border border-black/10 bg-white p-5 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_40px_rgba(15,23,42,0.10)]"
            >
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-neutral-500">
                Category
              </p>
              <p className="mt-3 text-2xl font-semibold text-neutral-950 transition group-hover:text-[color:var(--color-wsu)]">
                {item.name}
              </p>
      ...
      >
          ...
          <div className="flex flex-wrap gap-3">
            {collectionItems.map((item) => (
              <Link
                ...
                className="inline-flex items-center gap-2 rounded-full border border-neutral-200 bg-neutral-50 px-4 py-2 text-sm text-neutral-700 transition hover:-translate-y-0.5 hover:border-neutral-300 hover:bg-white hover:text-neutral-950"
        ...
        <div className="rounded-[28px] border border-black/10 bg-[linear-gradient(180deg,#ffffff_0%,#fafaf9_100%)] p-6 shadow-sm">
          <p className="text-sm font-semibold uppercase tracking-[0.24em] text-[color:var(--color-wsu)]">
            Latest arrivals
          </p>
